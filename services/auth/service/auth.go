...
func (s *Service) Register(ctx context.Context, req *connect.Request[v1.RegisterRequest]) (*v1.User, string, error) {
	input := req.Msg
	// Check if user already exists
	_, err := s.store.Queries.GetUserByEmail(ctx, input.Email)
	if err == nil {
		// User found - email already exists
		return nil, "", helper.RpcError(connect.CodeAlreadyExists, "Email already exists")
	}

	// If error is NOT "no rows found", something went wrong with the database
	if !errors.Is(err, pgx.ErrNoRows) {
		return nil, "", helper.RpcError(connect.CodeInternal, "Failed to check existing user")
	}

	// No user found (ErrNoRows) - proceed with registration
...
func (s *Service) Login(ctx context.Context, req *connect.Request[v1.LoginRequest]) (string, *v1.User, error) {
	input := req.Msg
...
func (s *Service) Verify(ctx context.Context, req *connect.Request[v1.VerifyRequest]) (string, *v1.User, error) {
	input := req.Msg
